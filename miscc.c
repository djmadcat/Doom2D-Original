...
void Z_drawspr(int x, int y, void *p, char d) {
    if (d) {
        V_spr2(x - w_x + 100, y - w_y + HT / 2 + 1 + w_o, p);
    } else {
        V_spr(x - w_x + 100, y - w_y + HT / 2 + 1 + w_o, p);
    }
}
...
void Z_drawstprcnt(int y, int n) {
    ...
    V_setrect(200, 70, y * 19 + 7 + w_o, 19);
    Z_clrst();
    sprintf(s, "%3d%%", n);
    l = strlen(s);
    x = 210;
...
void Z_drawstwpn(int n, int a) {
    ...
    i = n;
    V_setrect(200, 120, w_o + 58, 23);
    Z_clrst();
    if (i >= 0) {
        V_spr(232, w_o + 58 + 19, sth[i + 12]);
    }
    if (n >= 2) {
        sprintf(s, "%d", a);
        l = strlen(s);
...
void Z_drawmanspr(int x, int y, void *p, char d, byte color) {
    if (d) {
        V_manspr2(x - w_x + 100, y - w_y + HT / 2 + 1 + w_o, p, color);
    } else {
        V_manspr(x - w_x + 100, y - w_y + HT / 2 + 1 + w_o, p, color);
    }
}
