...
extern int doom_sfnt;
extern int doom_bfnt;
...
extern vgaimg flame01, flame02, flame03, flame04, flame05, flame06, flame07, flame08;

static vgaimg *fspr[8] = {
    &flame01, &flame02, &flame03, &flame04, &flame05, &flame06, &flame07, &flame08
};
...
void flame_act(void) {
    ...
    for (i = 0; i < MAXFLM; ++i) {
        if (flm[i].s >= 0) {
            if (flm[i].xv) {
                flm[i].xv += (flm[i].xv > 0) ? -1 : 1;
            }
            if (flm[i].yv) {
                flm[i].yv += (flm[i].yv > 0) ? -1 : 1;
            }
            flm[i].x += flm[i].xv;
            flm[i].y += flm[i].yv;
            if (++flm[i].s >= 8) {
...
void vputs(char *s) {
    ...
    if (vputs_flm) {
        ...
        if (rand() & 1) {
            new_fmov(vf_x, vf_y + h, vf_x + w, vf_y + h, 5, h);
        } else {
            new_fmov(vf_x + w, vf_y + h, vf_x, vf_y + h, 5, h);
        }
    }
    V_puts(s);
}
...
//    draw_back();
    ...
    switch (m) {
        ...
        case SOUND:
            ...
            gotoxy(100, 80);
            if (snd_card) {
                V_printf("äÄêíÄ %s", strupr(snd_name));
            } else {
                vputs("áÇìäéÇéâ äÄêíõ çÖí");
            }
            p = snd_par;
            gotoxy(100, 90);
            vputs("èéêí ");
            ...
            gotoxy(100, 120);
            V_printf("óÄëíéíÄ %u Éñ", sfreq);
            gotoxy(100, 130);
            vputs((snd_card) ? "èêéÇÖêäÄ" : "ìëíÄçéÇäÄ");
        ...
        case TESTYN:
            ...
            gotoxy(90, 60);
            vputs("èêéÑéãÜàíú èêéÇÖêäì ?");
            gotoxy(10, 80);
            vputs("(ùíé åéÜÖí èêàÇÖëíà ä áÄÇàëÄçàû äéåèúûíÖêÄ)");
            gotoxy(150, 100);
            vputs("çÖí");
            gotoxy(150, 110);
            vputs("ÑÄ");
            ...
            break;
        case SNDYN: gotoxy(100, 60);
            vputs("Çõ ëãõòÄãà áÇìä ?");
            gotoxy(150, 100);
            vputs("ÑÄ");
            gotoxy(150, 110);
            vputs("çÖí");
        ...
        case IDIR:
            ...
            gotoxy(20, 100);
            vputs(ddir);
            V_cursor(vf_x + 8, vf_y);
            break;
        case INST:
            V_prhdr(35, "àÑÖí ìëíÄçéÇäÄ");
            gotoxy(90, 70);
            vputs("F10 - éíåÖçÄ ìëíÄçéÇäà");
            break;
        case IDONE:
...
void setmnu(int m) {
    ...
    if (m == IDIR && mnu == SNDYN) {
        ...
        S_init();
        if (snd_type != ST_NONE) {
            if (snd_card != 3 && snd_card != 4) {
                if (load_dmm("install.dmm")) {
                    S_startmusic();
                }
            }
        }
        if (snd_type != ST_NONE) {
            S_play(ipath_snd, 7, 1024, 255);
        }
    }
    vputs_flm = 1;
    V_setscr(scrbuf);
...
void load_res(void) {
    ...
    if (!(ipath_snd = load_dmi("INSTPATH.DMI"))) {
        puts("ç• ¨Æ£„ Æ‚™‡Î‚Ï ‰†©´ INSTPATH.DMI");
        exit(1);
    }
    if (!(igo_snd = load_dmi("INSTGO.DMI"))) {
        puts("ç• ¨Æ£„ Æ‚™‡Î‚Ï ‰†©´ INSTGO.DMI");
        exit(1);
    }
    if (!(iend_snd = load_dmi("INSTEND.DMI"))) {
        puts("ç• ¨Æ£„ Æ‚™‡Î‚Ï ‰†©´ INSTEND.DMI");
        exit(1);
    }
}
...
int main(int ac, char *av[]) {
        ...
        if (swmnu) {
            ...
            if (swmnu == INST) {
                if (!ddir[0]) {
                    ...
                    if (ddir[i = strlen(ddir) - 1] == '\\') {
                        ddir[i] = 0;
                    }
                    build_dir(ddir);
                    inst_reset();
                    if (snd_type != ST_NONE) {
                        S_play(igo_snd, 7, 1024, 255);
                    }
                }
            }
            setmnu(swmnu);
            if (swmnu == TESTSND) {
                ...
                S_init();
                if (snd_type != ST_NONE) {
                    S_play(testsnd[rand() % TSNDS], 8, 1024, 255);
                }
                for (i = 0; i < 27; ++i) {
                    timer = 0;
                    main_loop();
    ...
    if (mnu == IDONE) {
        ...
        save_cfg();
        if (ddir[1] == ':') {
            _dos_setdrive(ddir[0] - 'A' + 1, (unsigned *) &j);
        }
        chdir(ddir);
