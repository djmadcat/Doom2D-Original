...
int wp_it[11] = {
    0, I_CSAW, 0, I_SGUN, I_SGUN2, I_MGUN, I_LAUN, I_PLAS, I_BFG, I_GUN2, 0
};
...
static byte dieanim[] = "HHHHIIIIJJJJKKKKLLLLMMMM";
static byte slopanim[] = "OOPPQQRRSSTTUUVVWW";

void PL_savegame(int h) {
    write(h, &pl1, sizeof(pl1));
    if (_2pl) {
        write(h, &pl2, sizeof(pl2));
    }
    write(h, &PL_JUMP, 4);
    write(h, &PL_RUN, 4);
    write(h, &p_immortal, 1);
}

void PL_loadgame(int h) {
    read(h, &pl1, sizeof(pl1));
    if (_2pl) {
        read(h, &pl2, sizeof(pl2));
    }
    read(h, &PL_JUMP, 4);
    read(h, &PL_RUN, 4);
    read(h, &p_immortal, 1);
...
static void fire(player_t *p) {
    ...
    if (keys[p->kf] || p->wpn == 8) {
        ...
        if (p->wpn == 10) {
            WP_ognemet(p->o.x, p->o.y - 15, p->o.x + ((p->d) ? 30 : -30), p->o.y - 15 + firediry(p), p->o.xv + p->o.vx, p->o.yv + p->o.vy, p->id);
        } else if (p->wpn >= 1) {
            ff[p->wpn](p->o.x, p->o.y - 15, p->o.x + ((p->d) ? 30 : -30), p->o.y - 15 + firediry(p), p->id);
        } else {
...
void PL_alloc(void) {
    int i, j;
    static char nm[][6] = {
        "OOF", "PLPAIN", "PLDETH", "SLOP", "PDIEHI", "BFG", "SAWUP", "SAWIDL", "SAWFUL", "SAWHIT", "PLFALL"
    };
    static char s[6];
...
void PL_act(player_t *p) {
    ...
    switch (p->st) {
        ...
        case OUT:
            p->o.xv = Z_dec(p->o.xv, 1);
            if (keys[p->ku] || keys[p->kd] || keys[p->kl] || keys[p->kr] || keys[p->kf] || keys[p->kj] || keys[p->kp] || keys[p->kwl] || keys[p->kwr]) {
                if (p->st != OUT) {
...
void PL_draw(player_t *p) {
    ...
    if (s) {
        Z_drawmanspr(p->o.x, p->o.y, spr[(s - 'A') * 2 + p->d], sprd[(s - 'A') * 2 + p->d], p->color);
    }
}
