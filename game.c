...
//            return;
//        }
//        VP_tocolor(std_pal, 0, 0, 0, 64, g_sttm);
//        VP_setall(pal_tmp);
        return;
    }
    if (GM_act()) {
...
/*
    if (chk == 1) {
        if (g_time > GETIME) {
            ...
            if (_2pl) {
                PL_hit(&pl2, 1, 0, HIT_SOME);
            }
            if (!_2pl) {
                if (PL_isdead(&pl1)) {
                    chk_exit();
                }
            } else if (PL_isdead(&pl1) && PL_isdead(&pl2)) {
                chk_exit();
            }
        }
    }
...
static void pl_info(player_t *p, int y) {
    ...
    Z_printbf("ìÅàã");
    Z_gotoxy(25, y + 15);
    Z_printbf("ìÅàâëíÇ Ç åàçìíì");
    Z_gotoxy(25, y + 30);
    Z_printbf("çÄòÖã ëÖäêÖíéÇ %u àá %u", p->secrets, sw_secrets);
    Z_gotoxy(255, y);
    Z_printbf("%u", p->kills);
    Z_gotoxy(255, y + 15);
...
void G_draw(void) {
    ...
    switch (g_st) {
        ...
        case GS_INTER:
            V_pic(0, 0, scrnh[1]);
            Z_gotoxy(60, 20);
            Z_printbf("ìêéÇÖçú èêéâÑÖç");
            Z_calc_time(g_time, &hr, &mn, &sc);
            Z_gotoxy(115, 40);
            Z_printbf("áÄ %u:%02u:%02u", hr, mn, sc);
            h = 60;
            if (_2pl) {
                Z_gotoxy(80, h);
                Z_printbf("èÖêÇõâ àÉêéä");
                Z_gotoxy(80, h + 70);
                Z_printbf("Çíéêéâ àÉêéä");
                h += 20;
            }
            pl_info(&pl1, h);
            if (_2pl) {
    ...
    if (h) {
        V_maptoscr(0, 200, (_2pl) ? 1 : 51, 98, clrmap + h * 256);
    } else {
        V_copytoscr(0, 200, (_2pl) ? 1 : 51, 98);
    }
    if (pl1.drawst) {
        V_copytoscr(200, 120, (_2pl) ? 0 : 50, 100);
    }
    pl1.drawst = 0;
    if (_2pl) {
        if (pl2.invl) {
        ...
        if (h) {
            V_maptoscr(0, 200, 101, 98, clrmap + h * 256);
        } else {
            V_copytoscr(0, 200, 101, 98);
        }
        if (pl2.drawst) {
