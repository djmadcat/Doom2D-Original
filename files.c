...
//    if (d_end < d_start) {
//        ERR_failinit("D_END is before D_START");
//    }
//    if (m_end < m_start) {
//        ERR_failinit("M_END is before M_START");
//    }
/*
    for (i = 0; i < wad_num; ++i) {
        restab[i].l = 0;
        ...
        if (memicmp(wad[i].n, "PLAYPAL", 8) == 0) {
            continue;
        }
        if (memicmp(wad[i].n, "COLORMAP", 8) == 0) {
            continue;
        }
        if (memicmp(wad[i].n, "MAP", 3) == 0) {
            continue;
        }
        if (!w_horiz && memicmp(wad[i].n, "RSKY", 4) == 0) {
            continue;
        }
        ...
        if (i > d_start && i < d_end) {
            if (snd_type == -1) {
                continue;
            } else if ((snd_type == 0 || snd_type == 1) && wad[i].n[1] != 'S') {
                continue;
            }
        }
        restab[i].l = wad[i].l;
