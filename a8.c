...
static unsigned char *frp;
static unsigned char sqc[2][50][80];
static int frame;
static a8_hdr_t ah;
static snd_t *strk;
static int strk_ch;
...
int A8_nextframe(void) {
    ...
    if (credits) {
        if (keys[0x33] && keys[0x34]) {
            skipping = 1;
        }
    }
    if (frame == -1) {
        if (strk) {
            strk_ch = S_play(strk, -1, 1024, 255);
        }
    }
    if (fdata) {
        len = *(int *) (fdata + fdptr);
...
int A8_start(char *nm) {
    ...
    end_clr = 1;
    if (stricmp(nm, "FINAL") == 0 || stricmp(nm, "CREDITS") == 0) {
        end_clr = 0;
    } else if (stricmp(nm, "KONEC") == 0) {
        end_clr = 0;
    }
    credits = (stricmp(nm, "FINAL") == 0) ? 1 : 0;
    if (stricmp(nm, "CREDITS") == 0) {
        if (skipping) {
            return 0;
        }
    }
    ...
    if (snd_type != ST_NONE) {
        ...
        if ((h = open(s, O_BINARY | O_RDONLY)) != -1) {
            ...
            if ((strk = malloc(sz + sizeof(snd_t))) != NULL) {
                ...
                for (p = (unsigned char *) (strk + 1); sz; --sz, ++p) {
                    *p ^= 0x80;
                }
            }
            close(h);
...
//        return 0;
    ...
    if (ah.id != A8_ID || ah.ver != 0) {
        ERR_fatal("Испорченный файл A8 %s", s);
    }
    lseek(fh, 0, SEEK_END);
