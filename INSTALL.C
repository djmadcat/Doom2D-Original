...
#define TSNDS 10
snd *testsnd[TSNDS];
snd *ipath_snd,*igo_snd,*iend_snd;
...
char *keyname(short k) {
  static char *nm[83]={
    ...
    "Q","W","E","R","T","Y","U","I","O","P","[","]","ENTER","ãÖÇõâ CONTROL","A","S",
    "D","F","G","H","J","K","L",";","\"","\'","ãÖÇõâ SHIFT","\\","Z","X","C","V",
    "B","N","M","<",">","/","èêÄÇõâ SHIFT","*","ãÖÇõâ ALT","èêéÅÖã","CAPS LOCK","F1","F2","F3","F4","F5",
    "F6","F7","F8","F9","F10","NUM LOCK","SCROLL LOCK","[7]","[8]","[9]","ëÖêõâ -","[4]","[5]","[6]","ëÖêõâ +","[1]",
    "[2]","[3]","[0]","[.]"
  };
  static char s[20];
  ...
  switch(k) {
    case 0x9C: return "ëÖêõâ ENTER";
    case 0x9D: return "èêÄÇõâ CONTROL";
    case 0xB8: return "èêÄÇõâ ALT";
    case 0xC7: return "HOME";
    case 0xC8: return "ÇÇÖêï";
    case 0xC9: return "PAGE UP";
    case 0xCB: return "ÇãÖÇé";
    case 0xCD: return "ÇèêÄÇé";
    case 0xCF: return "END";
    case 0xD0: return "Ççàá";
    case 0xD1: return "PAGE DOWN";
    case 0xD2: return "INSERT";
    case 0xD3: return "DELETE";
  }
  sprintf(s,"äãÄÇàòÄ #%X",k);
  return s;
}
...
void drawmenu(int m) {
  ...
  int i;
  static char *kn[9]={
    "ÇãÖÇé","ÇèêÄÇé","ÇÇÖêï","Ççàá","èêõÜéä","éÉéçú",
    "ëãÖÑ. éêìÜàÖ","èêÖÑ. éêìÜàÖ","éíäêõÇÄíú"
  };
...
//  draw_back();
  ...
  switch(m) {
    case MAIN:
      V_prhdr(35,"SETUP");
      prmenu(100,80,cur[m],"çÄëíêéâäÄ áÇìäÄ","ÇõÅéê äãÄÇàò","ÇõïéÑ Ç DOS",NULL);
      break;
    case SOUND:
      V_prhdr(35,"çÄëíêéâäÄ áÇìäÄ");
      gotoxy(100,80);
      if(snd_card) V_printf("äÄêíÄ %s",strupr(snd_name));
        else vputs("áÇìäéÇéâ äÄêíõ çÖí");
      p=snd_par;
      gotoxy(100,90);vputs("èéêí ");
      if(*p) V_printf("%X",snd_port); else vputs("çÖ çìÜÖç");
      p+=*p+1;
      gotoxy(100,100);vputs("DMA ");
      if(*p) V_printf("%u",snd_dma); else vputs("çÖ çìÜçé");
      p+=*p+1;
      gotoxy(100,110);vputs("IRQ ");
      if(*p) V_printf("%u",snd_irq); else vputs("çÖ çìÜçé");
      gotoxy(100,120);V_printf("óÄëíéíÄ %u Éñ",sfreq);
      gotoxy(100,130);vputs((snd_card)?"èêéÇÖêäÄ":"ìëíÄçéÇäÄ");
      V_cursor(100-10,80+cur[m]*10);
      break;
    case TESTSND:
      V_prhdr(90,"èêéÇÖêäÄ áÇìäÄ");
      break;
    case TESTYN:
      V_prhdr(35,"çÖí áÇìäéÇéâ äÄêíõ");
      gotoxy(90,60);vputs("èêéÑéãÜàíú èêéÇÖêäì ?");
      gotoxy(10,80);vputs("(ùíé åéÜÖí èêàÇÖëíà ä áÄÇàëÄçàû äéåèúûíÖêÄ)");
      gotoxy(150,100);vputs("çÖí");
      gotoxy(150,110);vputs("ÑÄ");
      V_cursor(150-10,100+cur[m]*10);
      break;
    case SNDYN:
      gotoxy(100,60);vputs("Çõ ëãõòÄãà áÇìä ?");
      gotoxy(150,100);vputs("ÑÄ");
      gotoxy(150,110);vputs("çÖí");
      V_cursor(150-10,100+cur[m]*10);
      break;
    case SCARD:
      V_prhdr(35,"ÇõÅÖêà áÇìäéÇìû äÄêíì");
      for(i=0;i<SDRV__END;++i) {
        set_snd_drv(i);
        gotoxy(100,80+i*10);
        vputs((i==0)?"ÅÖá áÇìäÄ":strupr(snd_name));
      }
      V_cursor(100-10,80+cur[m]*10);
      set_snd_drv(snd_card);
      break;
    case SPORT:
      V_prhdr(35,"ÇõÅÖêà èéêí");
      p=snd_par+1;
    ...
    case SIRQ:
      ...
      break;
    case SFREQ:
      V_prhdr(35,"ÇõÅÖêà äÄóÖëíÇé áÇìäÄ");
      prmenu(130,80,cur[m],
        "5000 Éñ","6000 Éñ","7000 Éñ","8000 Éñ","9000 Éñ",
        "10000 Éñ","11000 Éñ","16000 Éñ","22000 Éñ","44000 Éñ",NULL);
      break;
    case RKPL:
      V_prhdr(35,"ÇõÅéê äãÄÇàò");
      prmenu(100,80,cur[m],"èÖêÇõâ àÉêéä","Çíéêéâ àÉêéä",NULL);
      break;
    case RKEYS:
      V_prhdr(35,"ÇõÅÖêà äãÄÇàòà");
      for(i=0;i<9;++i) {
      ...
      break;
    case IDIR:
      V_prhdr(35,"äìÑÄ ìëíÄçÄÇãàÇÄíú");
      gotoxy(20,100);vputs(ddir);V_cursor(vf_x+8,vf_y);
      break;
    case INST:
      V_prhdr(35,"àÑÖí ìëíÄçéÇäÄ");
      gotoxy(90,70);vputs("F10 - éíåÖçÄ ìëíÄçéÇäà");
      break;
    case IDONE:
      V_prhdr(90,"ìëíÄçéÇäÄ áÄÇÖêòÖçÄ");
      break;
  }
}
...
void load_res(void) {
  ...
  if(!(ipath_snd=load_dmi("INSTPATH.DMI")))
    {puts("ç• ¨Æ£„ Æ‚™‡Î‚Ï ‰†©´ INSTPATH.DMI");exit(1);}
  if(!(igo_snd=load_dmi("INSTGO.DMI")))
    {puts("ç• ¨Æ£„ Æ‚™‡Î‚Ï ‰†©´ INSTGO.DMI");exit(1);}
  if(!(iend_snd=load_dmi("INSTEND.DMI")))
    {puts("ç• ¨Æ£„ Æ‚™‡Î‚Ï ‰†©´ INSTEND.DMI");exit(1);}
}
