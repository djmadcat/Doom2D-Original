    ...
    for (i = e = 0; i < dn; ++i) {
        if (dd[i].len == len) {
            ...
            for (l = len, e = 0; l;) {
                n = min(l, 32000L);
                read(h, buf, n);
                if (read(wadh, buf + 32000, n) != n) {
                    puts("!!! read error");
                }
                l -= n;
                if (memcmp(buf, buf + 32000, n) != 0) {
                    e = 0;
                    break;
                } else {
                    e = 1;
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 3) {
        puts("Usage: DMM2WAD listfile.lst wadfile.wad [start_number]");
        return 1;
    }
    if (argc >= 4) {
        fdmi = atoi(argv[3]);
