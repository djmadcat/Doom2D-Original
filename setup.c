...
char *keyname(short k) {
  static char *nm[83]={
    ...
    "Q","W","E","R","T","Y","U","I","O","P","[","]","ENTER","‹…‚›‰ CONTROL","A","S",
    "D","F","G","H","J","K","L",";","\"","\'","‹…‚›‰ SHIFT","\\","Z","X","C","V",
    "B","N","M","<",">","/","€‚›‰ SHIFT","*","‹…‚›‰ ALT","Ž…‹","CAPS LOCK","F1","F2","F3","F4","F5",
  ...
  switch(k) {
    case 0x9C: return "‘…›‰ ENTER";
    case 0x9D: return "€‚›‰ CONTROL";
...
void drawmenu(int m) {
  ...
  switch(m) {
    ...
    case SOUND:
      V_prhdr(35,"€‘’Ž‰Š€ ‡‚“Š€");
      gotoxy(100,80);V_printf("Š€’€ %s",strupr(snd_name));
      p=snd_par;
      gotoxy(100,90);V_puts("Ž’ ");
      if(*p) V_printf("%X",snd_port); else V_puts("… “†…");
      p+=*p+1;
      gotoxy(100,100);V_puts("DMA ");
      if(*p) V_printf("%u",snd_dma); else V_puts("… “†Ž");
      p+=*p+1;
      gotoxy(100,110);V_puts("IRQ ");
      if(*p) V_printf("%u",snd_irq); else V_puts("… “†Ž");
      gotoxy(100,120);V_printf("—€‘’Ž’€ %u ƒ–",sfreq);
      gotoxy(100,130);V_puts("Ž‚…Š€");
    ...
    case TESTYN:
      V_prhdr(35,"…’ ‡‚“ŠŽ‚Ž‰ Š€’›");
      gotoxy(90,60);V_puts("Ž„Ž‹†ˆ’œ Ž‚…Š“ ?");
      gotoxy(10,80);V_puts("(’Ž ŒŽ†…’ ˆ‚…‘’ˆ Š ‡€‚ˆ‘€ˆž ŠŽŒœž’…€)");
      gotoxy(150,100);V_puts("…’");
      gotoxy(150,110);V_puts("„€");
      V_spr(150-10,100+cur[m]*10,&cursor);
      break;
    case SNDYN:
      gotoxy(100,60);V_puts("‚› ‘‹›˜€‹ˆ ‡‚“Š ?");
      gotoxy(150,100);V_puts("„€");
      gotoxy(150,110);V_puts("…’");
      V_spr(150-10,100+cur[m]*10,&cursor);
      break;
