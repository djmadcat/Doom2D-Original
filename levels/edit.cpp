...
static textstr sw_cn(45, 125, 0, 5, "", &smallfont, 12);
static chkbox thf_r(115, 135, 10, 9, E_THF_R, 0);
static chkbox thf_dm(115, 145, 10, 9, E_THF_DM, 0);
static chkbox swf_pp(25, 155, 10, 9, E_SWF_PP, 0);
static chkbox swf_mp(25, 165, 10, 9, E_SWF_MP, 0);
static chkbox swf_pn(25, 175, 10, 9, E_SWF_PN, 0);
static chkbox swf_mn(25, 185, 10, 9, E_SWF_MN, 0);
static chkbox swf_kr(100, 155, 10, 9, E_SWF_KR, 0);
static chkbox swf_kg(100, 165, 10, 9, E_SWF_KG, 0);
static chkbox swf_kb(100, 175, 10, 9, E_SWF_KB, 0);
static strlist2 wl1(25, 125, 60, 70, walls, E_WALLIMG, 8);
static strlist2 wl2(25, 125, 60, 70, walls, E_WALLIMG, 8);
static strlist2 wl3(25, 125, 60, 70, walls, E_WALLIMG, 8);
...
static void redraw_th(void) {
    th_cn.newtext(115, 127, 0, 5, (curth == -1) ? "" : TH_getname(TH_gett(curth)), &smallfont, 12);
    int i = TH_getf(curth);
    thf_r.st = i & THF_DIR;
    thf_r.redraw();
    thf_dm.st = i & THF_DM;
...
static void redraw_sw(void) {
    ...
    byte f = SW_getf();
    swf_pp.st = f & SW_PL_PRESS;
    swf_pp.redraw();
    swf_mp.st = f & SW_MN_PRESS;
    swf_mp.redraw();
    swf_pn.st = f & SW_PL_NEAR;
    swf_pn.redraw();
    swf_mn.st = f & SW_MN_NEAR;
    swf_mn.redraw();
    swf_kr.st = f & SW_KEY_R;
    swf_kr.redraw();
    swf_kg.st = f & SW_KEY_G;
    swf_kg.redraw();
    swf_kb.st = f & SW_KEY_B;
    swf_kb.redraw();
}
...
add_wall(void) {
    ...
    button b1(194, 140, 30, 10, "О'кей", C_OK, 0, 13), b2(226, 140, 30, 10, "Не надо", C_CANCEL, 0, 27);
    ...
    for (msg = 0; !msg || msg == C_YES;) {
        GUI();
        if (msg == C_YES) {
            ib.cur = strlen(strcpy(buf, p[par]));
            ib.redraw();
        }
        if (lc != ib.cur || msg == C_YES) {
            M_unlock(sv1.img);
            sv1.img = (vgaimg *) M_lock(F_findres(buf));
            sv1.redraw();
            lc = ib.cur;
        }
        if (msg == C_YES) {
...
static void set_music(void) {
    ...
    button b1(129, 140, 30, 10, "О'кей", C_OK, 0, 13), b2(161, 140, 30, 10, "Не надо", C_CANCEL, 0, 27);

    memcpy(buf, musname, 8);
    buf[8] = 0;
...
int choose_file(char *dir, char **mask, char *file, char *title) {
    ...
    button b1(115, 175, 40, 10, "О'кей", C_OK, 0, 13), b2(165, 175, 40, 10, "Не надо", C_CANCEL, 0, 27);
    textstr t1(160, 15, 1, 5, title, &smallfont, 15), tdir(160, 25, 1, 5, dir, &smallfont, 12);
        ...
        if (msg == C_YES) {
            ...
            if (p[par][0] == '<') {
                ...
                *file = 0;
                ...
                ib.cur = strlen(strcpy(file, p[par]));
                ib.redraw();
            }
        }
    }
    ...
    Z_freelist(p);
    chdir(cdir);
    if (msg == C_CANCEL || file[0] == 0) {
        return 0;
    }
    if (f_path[strlen(strcpy(f_path, dir)) - 1] != '\\') {
        strcat(f_path, "\\");
    }
    strcat(f_path, file);
    return 1;
...
void desk::command(int c, long p) {
    ...
    static menu_t sysmnu[] = {
        "Опции", E_OPTIONS, "Справка", E_ABOUT, "Очистка", E_NEWMAP, "Небо", E_SETSKY, "Выход", E_QUIT, NULL, 0
    };
    static byte swf[7] = {
        SW_PL_PRESS, SW_MN_PRESS, SW_PL_NEAR, SW_MN_NEAR, SW_KEY_R, SW_KEY_G, SW_KEY_B
    };
    ...
    switch (c) {
        ...
        case E_DSWITCH:
            ...
            break;
        case E_SYSTEM:
            desktop <<= *new menubox(305, 2, sysmnu);
            desktop.redraw();
            break;
        case E_QUIT:
            if (message(MB_YN, "   Уже уходите?") == C_YES) {
                ERR_quit();
            }
            break;
        case E_OPTIONS:
        ...
        case E_GAMMA:
            ...
            break;
        case E_ETYPE:
            send_command(&vw, E_DBACK + p, 1);
            etype = p;
            break;
        case E_WALLIMG:
            if (etype < 3) {
                wallimg[etype] = p + 1;
                wlsv[etype]->img = (p >= -1) ? walp[p + 1] : NULL;
                wlsv[etype]->redraw();
                if (p <= -2) {
                    wl2.lst->cline = -1;
                    wl2.redraw();
        ...
        case E_MUSIC:
            ...
            break;
        case E_NEWMAP:
            if (message(MB_YN, "Стереть весь уровень?") != C_YES) {
                break;
            }
            if (message(MB_YN, "Вы уверены?") != C_YES) {
                break;
            }
            W_init();
            rebuild_wl();
            ...
            break;
        case E_LOADMAP:
            if (!choose_file(mapdir, mapmask, mapfile, "ЗАГРУЗКА КАРТЫ")) {
                break;
            }
            if (!(s = choose_map(f_path, "ЗАГРУЗКА КАРТЫ"))) {
                break;
            }
            W_init();
            F_loadmap(f_path, s);
            ...
            break;
        case E_SAVEMAP:
            if (!choose_file(mapdir, mapmask, mapfile, "ЗАПИСЬ КАРТЫ")) {
                break;
            }
//            if (access(f_path, 0) == 0) {
//                if (message(MB_YN, "Стереть старый %s ?", f_path) != C_YES) {
//                    break;
//                }
//            }
            if (!(s = choose_map(f_path, "ЗАПИСЬ КАРТЫ"))) {
                break;
            }
        ...
        case E_HELP:
            switch (p) {
                ...
                case H_FRONT:
                    ...
                    break;
                case H_WALL:
                    message(MB_OK, "Выберите тип стены.\n"
                    ...
                    break;
                case H_THING:
                    message(MB_OK, "Чтобы поставить новый предмет, выберите его из списка\n"
                    ...
                    break;
                case H_SWITCH:
                    message(MB_OK, "Чтобы поставить новую кнопку, в нужном месте на виде\n"
...
view::view() : gui_obj(0, 0, 250, 110) {
    *this <<= *(hs = new hscroll(0, 100, 200, 10, FLDW * CELW - 200, FLDW * CELW / 2, E_VIEWX, 8)) += *(vs = new vscroll(200, 0, 10, 100, FLDH * CELH - 98, FLDH * CELH / 2, E_VIEWY, 8));
    for (int i = 0; i < 5; ++i) {
...
void view::draw(void) {
    static vgaimg *s[5] = { &back_icon, &wall_icon, &front_icon, &thing_icon, &switch_icon };
...
int view::mouse(int x, int y, int b) {
    ...
    switch (etype) {
        ...
        case EFRONT:
            ...
            edt = b;
            edx1 = edx2 = (x + w_x - 100) / 8;
            edy1 = edy2 = (y + w_y - 50) / 8;
            redraw();
            while (I_mget(&x, &y) & b) {
                if (x >= 200) {
                    x = 199;
            ...
            if (b == 1) {
                ...
                b = 0;
                i = (wallimg[etype] >= 0) ? 0 : -1;
            }
            sx = sy = 1;
            if (i > 0) {
                if (((unsigned) walp[i]) > 16) {
                    sx = (walp[i]->w + 7) / 8;
                    sy = (walp[i]->h + 7) / 8;
                }
            }
            for (edx2 = edx2 - edx1 + 1, ly = 0; edy1 <= edy2; ++edy1, ++ly) {
                if (i > 0) {
                    ...
                    memset(p + edy1 * FLDW + edx1, 0, edx2);
                }
                if (etype == EWALL) {
                    memset(&fld[edy1][edx1], b, edx2);
                }
            }
            edt = 0;
            redraw();
        ...
        case ETHING:
            if ((i = TH_isthing(w_x - 100 + x, w_y - 50 + y)) != -1) {
                ...
                curth = TH_add(w_x - 100 + x, w_y - 50 + y, pthing);
            }
            redraw_th();
            desktop.redraw();
            while (I_mget(&x, &y)) {
...
int mapwin::mouse(int x, int y, int b) {
    ...
    while (I_mget(&x, &y)) {
        ...
        vw.hs->sld->cur = x * 8 - 100;
        vw.hs->sld->adjust();
        vw.vs->sld->cur = y * 8 - 49;
        vw.vs->sld->adjust();
        GUI_wait();
    }
    return 1;
...
void sprview::draw(void) {
    ...
    if (scrx + w > left + width) {
        scrw = left + width - scrx;
    }
    if (scry + h > top + height) {
        scrh = top + height - scry;
    }
    V_clr(left, width, top, height, gray[color]);
    if (img) {
        if (img->w <= 320 && img->h <= 200) {
            V_spr(left + img->sx, top + img->sy, img);
        }
    }
    scrw = w;
    scrh = h;
...
extern "C" void edit(void) {
    static textstr t1(115, 115, 1, 5, "ФОН", &smallfont, 15), t2(115, 115, 1, 5, "СТЕНЫ, ДВЕРИ И Т.П.", &smallfont, 15), t3(115, 115, 1, 5, "ПЕРЕДНИЙ ПЛАН", &smallfont, 15), t4(115, 115, 1, 5, "ВЕЩИ, МОНСТРЫ И Т.П.", &smallfont, 15), t5(115, 115, 1, 5, "КНОПКИ", &smallfont, 15), tht1(130, 137, 0, 5, "вправо", &smallfont, 15), tht2(130, 147, 0, 5, "только в DM", &smallfont, 15), swt1(40, 157, 0, 5, "игрок наж.", &smallfont, 15), swt2(40, 167, 0, 5, "монстр наж.", &smallfont, 15), swt3(40, 177, 0, 5, "игрок близко", &smallfont, 15), swt4(40, 187, 0, 5, "монстр близко", &smallfont, 15), swt5(115, 157, 0, 5, "красный ключ", &smallfont, 15), swt6(115, 167, 0, 5, "зеленый ключ", &smallfont, 15), swt7(115, 177, 0, 5, "синий ключ", &smallfont, 15);
    static sprbutton hb1(199, 113, 6, 7, &help_icon, E_HELP, H_BACK, 0x3B00), hb2(199, 113, 6, 7, &help_icon, E_HELP, H_WALL, 0x3B00), hb3(199, 113, 6, 7, &help_icon, E_HELP, H_FRONT, 0x3B00), hb4(199, 113, 6, 7, &help_icon, E_HELP, H_THING, 0x3B00), hb5(199, 113, 6, 7, &help_icon, E_HELP, H_SWITCH, 0x3B00), hb6(120, 123, 6, 7, &help_icon, E_HELP, H_SWTYPE, 0x3B00);
    static button baw1(145, 125, 60, 10, "Добавить", E_ADDWALL, 0, -1), baw2(155, 125, 50, 10, "Добавить", E_ADDWALL, 0, -1), baw3(145, 125, 60, 10, "Добавить", E_ADDWALL, 0, -1), bsw1(25, 123, 9, 9, "\x1B", E_PREVSW, 0, -1), bsw2(34, 123, 9, 9, "\x1A", E_NEXTSW, 0, -1), bsw3(25, 140, 40, 9, "Убрать", E_DELSW, 0, -1), bnw1(25, 114, 9, 9, "", E_WALLIMG, -2, -1);
    static strlist2 thl1(25, 125, 85, 70, thtn_list, E_THTYPE, 8);
    static cont_t cnt1[] = {
        &t1, &back_icon, &t2, &build_icon, &t3, &front_icon, &t4, &thing_icon, &t5, &switch_icon, NULL, NULL
    };
    static strlist2 wtl1(90, 125, 60, 70, wt_name + 1, E_WTYPE, 4);
    static sprbutton b1(305, 2, 13, 10, &system_icon, E_SYSTEM, 0, -1), b2(270, 10, 16, 15, &load_icon, E_LOADMAP, 0, 0x3D00), b3(270, 40, 16, 15, &save_icon, E_SAVEMAP, 0, 0x3C00), b5(308, 15, 6, 7, &help_icon, E_HELP, H_MAIN, 0x3B00);
    ...
    t5 += bsw1 += bsw2 += bsw3 += sw_cn += swt1 += swt2 += swt3 += swt4 += swt5 += swt6 += swt7 += swf_pp += swf_mp += swf_pn += swf_mn += swf_kr += swf_kg += swf_kb += hb5 += hb6;
    for (;;) {
        GUI();
    }
}
