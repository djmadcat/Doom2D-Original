...
void F_allocres(void) {
    ...
    if (d_end < d_start) {
        ERR_failinit("D_END до D_START");
    }
    if (m_end < m_start) {
        ERR_failinit("M_END до M_START");
    }
    if (s_end < s_start) {
        ERR_failinit("S_END до S_START");
    }
    if (w_end < w_start) {
        ERR_failinit("W_END до W_START");
    }
}
...
int F_savemap(char *fn, char *wn) {
    ...
    if ((savh = open(fn, O_BINARY | O_CREAT | O_TRUNC | O_RDWR, S_IREAD | S_IWRITE)) == -1) {
        goto err;
    }
    write(savh, "PWAD", 12);
    if (h != -1) {
...
char **F_make_file_list(char *dir, char **mask) {
    ...
    for (; *mask; ++mask) {
        if (f_path[strlen(strcpy(f_path, dir)) - 1] != '\\')
            strcat(f_path, "\\");
        strcat(f_path, *mask);
        for (dirp = opendir(f_path); ff = readdir(dirp);) {
            if (!(ff->d_attr & _A_SUBDIR)) {
