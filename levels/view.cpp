...
extern "C" void W_draw(void) {
    ...
    V_clr(0, 200, 0, 1, 0x6C);
    V_clr(0, 200, 99, 1, 0x6C);
    x = scrx;
    y = scry;
    w = scrw;
    h = scrh;
    ...
    V_pic(127 - (word) (w_x - WD / 2) * 56U / (word) (MAXX - WD / 2), w_o + 123 - (word) (w_y - HT / 2) * 28U / (word) (MAXY - HT / 2), (vgaimg *) horiz);
    if (w_drawb) {
        Z_drawfld((byte *) fldb);
    }
    if (w_drawth) {
    ...
    if (edt) {
        V_remap_rect(min(edx1, edx2) * 8 - w_x + 100, abs(edx2 - edx1) * 8 + 8, min(edy1, edy2) * 8 - w_y + 50, abs(edy2 - edy1) * 8 + 8, clrmap[6]);
    }
    scrx = x;
    scry = y;
    scrw = w;
...
extern "C" void W_addwall(char *n) {
    ...
    if (memicmp(n, "_WATER_", 7) == 0) {
        walp[i + 1] = (void *) ((int) n[7] - '0' + 1);
    } else if ((walp[i + 1] = M_lock(F_findres(n))) == NULL) {
        return;
    }
    memset(wal[i].n, 0, 8);
    strncpy(wal[i].n, n, 8);
    if ((dword) walp[i + 1] < 16) {
        return;
    }
    wal[i].t = 0;
    walf[i + 1] = 0;
...
extern "C" void W_drawwl(int x, int y, byte t) {
    ...
    if (walf[t] & 1) {
        V_spr(x, y, (vgaimg *) walp[t]);
    } else {
        V_pic(x, y, (vgaimg *) walp[t]);
    }
}
...
extern "C" void W_allocwalls(void) {
    ...
    for (i = 0; i < 255 && wal[i].n[0]; ++i) {
        if (memicmp(wal[i].n, "_WATER_", 7) == 0) {
            walp[i + 1] = (void *) ((int) wal[i].n[7] - '0' + 1);
            continue;
        }
